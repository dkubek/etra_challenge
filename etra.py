...
def read_data(data_path: Union[str, Path]) -> pd.DataFrame:
    ...
    participant_id, trial_id, fv_fixation, task_type, stimulus_id = (
        filename.split("_")[:5]
    )

    df = pd.read_csv(data_path)
    orig_cols = df.columns.to_list()
    new_cols = {'participant_id': participant_id, 'trial_id': trial_id,
                'fv_fixation': fv_fixation, 'task_type': task_type,
                'stimulus_id': stimulus_id, }
    df = df.assign(**new_cols)
    df = df[
        ["participant_id", "trial_id", "fv_fixation", "task_type",
         "stimulus_id"] + orig_cols
        ]
...
def detect(data):
    ...
    events = pd.DataFrame(
        events,
        columns=['label', 'start_time', 'end_time', 'start_x', 'start_y',
                 'end_x', 'end_y', 'amp',
